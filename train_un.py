        ...
        for epoch in range(1, self.epoch + 1):
            ...
            for i, (data, labels) in enumerate(pbar):
                ...
                if uncertainty:
                    ...
                    mean_prediction = torch.mean(preds, dim=0)
                    std_prediction = torch.std(preds, dim=0)
                    # print(std_prediction)s
                ...
                if uncertainty:
                    ...
                    loss += weight * torch.mean(std_prediction ** 2)
                    performance = compute_performance(pred, labels)
                    if std_prediction.mean() > weight_threshold and performance < 0.9:
                        weight *= weight_decay_rate
                    elif performance < 0.0001:
                        weight /= weight_decay_rate
                    # print(loss, mean_prediction, std_prediction)
                else:
                    loss = loss_function(pred, labels)
                    train_loss += loss.item()
