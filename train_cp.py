...
from model import MARS, CNN_LSTM, Transformer
...
class customLoss(torch.nn.Module):
    ...
    def forward(self, pred, labels, uncertainty):
        distance_error = pred - labels
        diff = torch.pow( (distance_error/uncertainty), 2 )
        un = torch.log(torch.pow(uncertainty,2))
        
        loss = torch.mean( torch.pow(distance_error, 2) ) + 0.001*torch.mean(diff+un)
        return loss
...
class Trainer():
    ...
    def train(self):
        ...
        #! test other model
        model = CNN_LSTM(globals()[self.args.model], self.input_size, self.hidden_size, self.num_layers, 39, self.dropout, self.bidirectional, self.device, self.window).to(self.device)
        # model = Transformer(src_vocab_size=64*9, tgt_vocab_size=39, d_model=5*8, num_heads=5, num_layers=6, d_ff=512, max_seq_length=2880, dropout=0.1, device=self.device).to(self.device)
        ...
        #!transformer
        # optimizer2 = torch.optim.Adam(transformer.parameters(), self.lr, weight_decay=self.weight_decay, betas=[0.5, 0.999], amsgrad=False)
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            '''Train'''
            train_loss_error_counter = 0
            test_loss_error_counter = 0
            model.train()
            ...
            for i, (data, labels) in enumerate(pbar):
                ...
                loss = loss_function(pred, labels, variance)
                # if loss < 0:
