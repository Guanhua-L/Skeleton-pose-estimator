...
class Trainer():
    ...
    def train(self):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            if uncertainty:
                log_header = ['uncertainty', 'max uncertainty', 'min uncertainty', 'Train loss error counter', 'Test loss error counter']
                # tmp = variance.tolist()
                # for i, t in enumerate(tmp):
                #     tmp[i] = f'{t:.10f}'
                log = [f'{variance.mean().item():.10f}', f'{variance.max().item():.10f}', f'{variance.min().item():.10f}', f'{train_loss_error_counter}', f'{test_loss_error_counter}']
